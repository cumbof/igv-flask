...
if __name__ == "__main__":
    ...
    # Check whether the input genome file exist
    if args.input_genome and not os.path.isfile(args.input_genome):
        raise FileNotFoundError(errno.ENOENT, os.strerror(errno.ENOENT), args.input_genome)
    ...
    # Initializing app
    app = Flask(TOOL_ID, template_folder=os.path.join(working_dir, "templates"))
    ...
    if args.input:
        # Add the path to the input fasta
        app.config["input"] = args.input

        # Session file is used in conjunction with an input genome only
        if args.igv_session:
